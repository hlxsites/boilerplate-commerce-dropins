...
import { r as regionsSignal, e as cartSignal } from "./transform-shipping-methods__Csm-KieitR.js";
import { o as AttributeFrontendInputEnum, V as ValidationRuleEnum, u as useAddressFormFields, p as useCountries, i as useStore, q as useServices } from "./countries__BNdKWz0f2q.js";
import { b as getMultilineValues, a as AddressFormFields, A as AddressFormTypes, e as estimateShippingMethods, D as DEBOUNCE_TIME, g as getRegions, M as MULTILINE_SUFFIX, c as MULTILINE_CUSTOM_ATTR_SPLIT } from "./estimateShippingMethods__BQV8ZoVRnA.js";
import { useState, useEffect, useCallback, useRef as useRef$1, useMemo } from "@dropins/tools/preact-hooks.js";
import { Text, useText } from "@dropins/tools/i18n.js";
import { Field, Input, Picker, Skeleton, SkeletonRow } from "@dropins/tools/components/index.js";
/* empty css                                  */
...
import { forwardRef, useRef, useImperativeHandle, useState as useState$1, useEffect as useEffect$1 } from "@dropins/tools/preact-compat.js";
...
function useDefaultValues(defaultCountry) {
  const [defaultValues, setDefaultValues] = useState$1({});
  useEffect$1(() => {
    if (!defaultCountry)
      return;
...
function useRegions({
  ...
  const [availableRegions, setAvailableRegions] = useState$1([]);
  useEffect$1(() => {
    if (!country) {
      setAvailableRegions([]);
      return;
...
function useFormFill({
  ...
  const [hasFormFillCompleted, setHasFormFillCompleted] = useState$1(false);
  const cartData = cartSignal.value.data;
  useEffect$1(() => {
    var _a;
